...
function makeTemplate(type: optionType, sheetName) {
	let index = (type == optionType.put) ? 0 : 1;
...
function makeTemplateSheet(template, sheetName) {
	...
	let regex1 = /[-()]/g;
	let namedRangePrefix = sheetName.replace(regex1, '_')
	...
	var regex = new RegExp(`${template.name}!`, 'g')
	var dataRow = template.datarow.map(val => val.replace(regex, `${namedRangePrefix}_`))
	//log(dataRow)
	let values = Array(3) //1000)
